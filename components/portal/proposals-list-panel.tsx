...
function proposalHubStageDisplay(p: ProposalRecord): { label: string; title: string; badgeKey: keyof typeof HUB_BADGE } {
    ...
    <div className="space-y-8">
      ...
      <section className="overflow-hidden rounded-xl border border-border/80 bg-card/80 shadow-sm backdrop-blur-sm">
        <div className="flex flex-col gap-3 border-b border-border px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          <h2 className="shrink-0 text-sm font-semibold text-foreground">All proposals</h2>
          ...
          <table className="w-full min-w-[720px] text-left text-[13px]">
            <thead>
              <tr className="border-b border-border text-muted-foreground">
                ...
                <th className="w-[148px] px-2 py-2.5 text-center font-medium">Actions</th>
              </tr>
            </thead>
                            ...
                            {p.shareToken ? (
                              <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                                <Link
                                  ...
                                  aria-label={`Open public view for “${p.title}”`}
                                >
